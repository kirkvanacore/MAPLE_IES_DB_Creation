#### CLEANING/AGGREGATING FH2T LOGS ####
...
problems <- read.csv("FH2T_Efficacy_Masterlist/worlds 1-14 Sep30-Table 1.csv")
...
table(assess$FH2T)
FH2T_users_check <- assess %>%
  filter(FH2T == 1) %>%
  select(StuID, FH2T,fidelity_started_sum, fidelity_complete_sum, SchIDPre, SchIDEnd) %>%
  full_join(logs %>%
              group_by(user_id) %>%
              summarise(num_problems = length(unique(problem_id)) ),
            by = c("StuID" = "user_id") # these are not the ids
            )
  
table(is.na(FH2T_users_check$num_problems))
table(is.na(FH2T_users_check$num_problems), FH2T_users_check$SchIDPre)
table(is.na(FH2T_users_check$num_problems), FH2T_users_check$SchIDEnd)

table(is.na(FH2T_users_check$num_problems), FH2T_users_check$fidelity_started_sum)
table(is.na(FH2T_users_check$num_problems), FH2T_users_check$fidelity_complete_sum)
...
#### AGGREGATED DATA AT THE ATTEMPT LEVEL ####
...
logs_cln <- logs_cln %>%
  ...
  dplyr::group_by(user_id, problem_id) %>% # this grouping is only for the lag section, 
  ...
  dplyr::group_by(user_id, problem_id, replay_attempt_num) %>%  
  dplyr::mutate(
    # number of resets within the 
    resets_within_replay_attempt = sum(reset)
    
  ) %>%
  dplyr::group_by(user_id, problem_id, attempt_num) %>%  
  dplyr::mutate(num_steps = sum(step),
                ...
                commpleted_dur_attempt = sum(completed_flag)
                ) %>%
...
spa <- logs_cln %>%
  ...
  summarise(
    ...
    commpleted_dur_attempt = max(commpleted_dur_attempt),
...
#### AGGREGATED DATA AT THE PROBLEM LEVEL ####
...
sp <- spa %>%
  ...
  dplyr::summarise(
    ...
    total_completed_attempts = sum(commpleted_dur_attempt),
  ...
  left_join(
    spa %>%
      group_by(user_id, problem_id) %>%
      dplyr::filter(commpleted_dur_attempt == 1) %>%
      dplyr::mutate(
    ...
    by = c("user_id" = "best_user_id", 
           "problem_id" = "best_problem_id")
  ) %>%
  left_join(
    spa %>%
      group_by(user_id, problem_id) %>%
      filter(commpleted_dur_attempt == 1) %>%
      dplyr::mutate(
...
head(as.POSIXct(logs_cln$time, origin = "1970-01-01"))
head(logs_cln$time)
